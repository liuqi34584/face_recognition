...
# 设置要生成文件的路径
data_root_path = './data/data2393/images/face'
# 所有类别的信息
class_detail = []
...
class_label = 0
# 获取总类别的名称
father_paths = data_root_path.split('/')    #['.', 'data', 'data2393', 'images', 'face']
while True:
...
data_list_path = './data/data2393/%s/' % father_path
# 如果不存在这个文件夹,就创建
...
BATCH_SIZE = 32
# 把图片数据生成reader
trainer_reader = train_r(train_list="./data/data2393/face/trainer.list")
train_reader = paddle.batch(
    paddle.reader.shuffle(
        reader=trainer_reader,buf_size=300),
    batch_size=BATCH_SIZE)

tester_reader = test_r(test_list="./data/data2393/face/test.list")
...
# image 和 label 是通过 fluid.layers.data 创建的两个输入数据层。
# 其中 image 是 [3, 100, 100] 维度的浮点数据; label 是 [1] 维度的整数数据。
# 
# 这里需要注意的是: Fluid中默认使用 -1 表示 batch size 维度，默认情况下会在 shape 的第一个维度添加 -1 。 
# 所以 上段代码中， 我们可以接受将一个 [-1, 3, 100, 100] 的numpy array传给 image 。
# Fluid中用来做类别标签的数据类型是 int64，并且标签从0开始。
...
# 搭建VGG网络
# 
# 1.首先定义了一组卷积网络，即conv_block。
# 卷积核大小为3x3，池化窗口大小为2x2，窗口滑动大小为2.
# groups决定每组VGG模块是几次连续的卷积操作，dropouts指定Dropout操作的概率。
# 所使用的img_conv_group是在paddle.networks中预定义的模块.
# 由若干组 Conv->BN->ReLu->Dropout 和 一组 Pooling 组成。
# 
# 2.五组卷积操作，即 5个conv_block。 
# 第一、二组采用两次连续的卷积操作。第三、四、五组采用三次连续的卷积操作。
# 每组最后一个卷积后面Dropout概率为0，即不使用Dropout操作。
# 
# 3.最后接两层512维的全连接。
# 
# 4.通过上面VGG网络提取高层特征，然后经过全连接层映射到类别维度大小的向量
# 再通过Softmax归一化得到每个类别的概率，也可称作分类器。
...
for pass_id in range(EPOCH_NUM):
    ...
    model_save_dir = "./data/data2393/model_vgg"
    # model_save_dir = "./data/data2393/model_cnn"
    # 如果保存路径不存在就创建
...
# 下面是预测程序，直接单独运行In[*]就可以。预测主要有四步：
# 第一步配置好预测的环境
# 第二步准备好要预测的图片
# 第三步加载预测的模型，把要预测的图片放到模型里进行预测
# 第四步输出预测的结果
...
#选择保存不同的训练模型
# params_dirname ="./data/data2393/model_cnn"
params_dirname ='./data/data2393/model_vgg'
